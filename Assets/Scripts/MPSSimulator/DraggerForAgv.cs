...
/// <summary>
/// AGV용 드래거 (통합 버전)
/// - 분배라인: Box 감지 시 이벤트 발생 → Jogging
/// - 리니어: SetReturning으로 복귀 모드 지원
/// </summary>
public class DraggerForAgv : MonoBehaviour
{
    [Header("목적지 설정")]
    ...
    // Box 감지 이벤트 (ConveyorForAgv에서 구독)
    ...
    public void SetReturning(bool returning)
    {
        ...
        if (showDebugLog)
        {
            Debug.Log($"<color=blue>[DraggerForAgv]</color> {gameObject.name} 복귀 모드: {returning}");
        }
    }
    ...
    public void ResetToCcwPosition()
    {
        if (ccwDestination != null)
        {
            ...
            if (showDebugLog)
            {
                Debug.Log($"<color=gray>[DraggerForAgv]</color> {gameObject.name} CCW 위치로 리셋");
            }
        }
    }
    ...
    public void ResetToCwPosition()
    {
        if (cwDestination != null)
        {
            ...
            if (showDebugLog)
            {
                Debug.Log($"<color=gray>[DraggerForAgv]</color> {gameObject.name} CW 위치로 리셋");
            }
        }
    }
    ...
    private void TryAttachBox(GameObject obj)
    {
        ...
        if (isBox && attachedBox == null)
        {
            ...
            if (showDebugLog)
            {
                Debug.Log($"<color=green>[DraggerForAgv]</color> {gameObject.name} Box 부착: {obj.name}");
            }
            
            // ConveyorForAgv에 Box 감지 알림
    ...
    public void DetachBox()
    {
        if (attachedBox != null)
        {
            ...
            if (showDebugLog)
            {
                Debug.Log($"<color=yellow>[DraggerForAgv]</color> {gameObject.name} Box 분리: {attachedBox.name}");
            }
        ...
        foreach (Transform child in transform)
        {
            if (child.CompareTag("Box_A") || child.tag.Contains("Box"))
            {
                if (showDebugLog)
                {
                    Debug.Log($"<color=yellow>[DraggerForAgv]</color> {gameObject.name} 자식 Box 분리: {child.name}");
                }
